...
static const value_string ws_net_dm_attrs_vals[] = {
	...
	{ WS_NET_DM_ATTR_SW,				"Software" },
	{ WS_NET_DM_ATTR_HW,				"Hardware" },
	{ 0, NULL },
};
...
static header_field_info hfi_net_dm_attrs_port NETLINK_NET_DM_HFI_INIT =
	{ "Attribute type", "net_dm.attr_port_type", FT_UINT16, BASE_DEC,
	  &ws_net_dm_attrs_port_vals, 0x00, NULL, HFILL };
...
static header_field_info hfi_net_dm_attrs_stats NETLINK_NET_DM_HFI_INIT =
	{ "Attribute type", "net_dm.attr_stats_type", FT_UINT16, BASE_DEC,
	  &ws_net_dm_attrs_stats_vals, 0x00, NULL, HFILL };

static header_field_info hfi_net_dm_origin NETLINK_NET_DM_HFI_INIT =
	{ "Packet origin", "net_dm.origin", FT_UINT16, BASE_DEC,
	  &ws_net_dm_origin_vals, 0x00, NULL, HFILL };
...
dissect_net_dm_attrs_port(tvbuff_t *tvb, void *data, proto_tree *tree, int nla_type, int offset, int len)
{
	...
	enum ws_net_dm_attrs_port type = (enum ws_net_dm_attrs_port) nla_type;
	const guint8 *str;
...
dissect_net_dm_attrs(tvbuff_t *tvb, void *data, proto_tree *tree, int nla_type, int offset, int len)
{
	...
	case WS_NET_DM_ATTR_PC:
		proto_item_append_text(tree, ": %p", (void *) tvb_get_letoh64(tvb, offset));
		proto_tree_add_item(tree, &hfi_net_dm_pc, tvb, offset, len, info->encoding);
		return 1;
	...
	case WS_NET_DM_ATTR_TIMESTAMP:
		proto_tree_add_item(tree, &hfi_net_dm_timestamp, tvb, offset, len, info->encoding);
		return 1;
...
proto_reg_handoff_netlink_net_dm(void)
{
	...
	eth_handle = find_dissector_add_dependency("eth_maybefcs", proto_netlink_net_dm);
}
